...
\begin{equation}
\{\mathbf{t}_m\} = \begin{bmatrix} X_m \\ Y_m \\ 0 \end{bmatrix}_O, \quad \{\mathbf{F}_m\} = \begin{bmatrix} X_F \\ Y_F \\ 0 \end{bmatrix}_I
\end{equation}
...
At joint $O$ (connecting links 1 and 2), force balance on the two-link system gives:
...
where $X_{r1}, Y_{r1}$ are reaction forces at joint 1, representing the interface between link 1 and the base.

For the second link at point $A$:
...
These equations represent static equilibrium when cable forces and gravity balance at each joint.
